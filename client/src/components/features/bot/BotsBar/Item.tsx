...
function Item({
  ...
  // Fallback to 'operational' if operationalStatus is missing (defensive coding)
  const operationalStatus = bot.operationalStatus || 'operational';
  const statusConfig = RobotOperationalStatusType[operationalStatus];

  // Generate tooltip content showing both operational health and assignment status
  const getTooltipContent = () => {
    const lines = [statusConfig.text];
