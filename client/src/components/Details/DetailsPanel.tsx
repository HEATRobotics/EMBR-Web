  ...
  return (
    <div className="relative h-full">
      ...
      <div className="relative">
        {/* black tab bar */}
        <div className="bg-black h-10 flex items-center justify-center rounded-t-md shadow-md relative z-20">
          {/* small handle in the center to emphasize the tab */}
          <div className="absolute left-1/2 transform -translate-x-1/2 top-1/2 -translate-y-1/2 w-14 h-2 rounded-full bg-gray-700 opacity-80" />
          {/* close button on the right of the tab */}
          <button
            onClick={onClose}
            aria-label="Close details panel"
            title="Close"
            className="absolute right-3 top-1/2 transform -translate-y-1/2 z-30 px-2 py-1 text-white text-xl font-bold rounded hover:bg-gray-800 transition-colors"
          >
            ×
          </button>
        </div>

        {/* Under the black tab we add a thin border to separate from content */}
        <div className="h-2 bg-transparent -mt-1" />
      </div>
      ...
      <div className="flex flex-col h-[calc(100%-3rem)] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {currentTab === "Bot Info" && <BotPanel selectedBot={selectedBot} onClose={onClose} />}
