  ...
  return (
    <div className="flex flex-col gap-y-3 w-full text-[20px] leading-6">
      <div className="flex flex-col py-5 px-[30px] gap-y-2.5 rounded-[22px] bg-white">
        ...
        {bots && (
          <>
            <StatLine name="Ready Bots" value={`${readyBots}/${totalBots} available`} />
            <StatLine name="Assigned Bots" value={`${assignedBots}/${totalBots} on missions`} />
          </>
        )}
        <StatLine
          name="Total Missions"
          value={`${missions.length} mission${missions.length === 1 ? '' : 's'} in the system`}
        />
      </div>
      <div className="flex flex-col gap-y-3 w-full overflow-auto h-full max-h-[calc(100vh-320px)]">
        {missions.map((mission, i) => (
          <div key={i} className="flex flex-col py-6 px-[30px] gap-y-2.5 rounded-[22px] bg-white">
            ...
            <StatLine name="# of smokes" value={`${mission.smokesDetected ?? 0} smokes detected`} />
            <StatLine
              name="Average Temp of the area"
              value={`${mission.averageTemperature || '-'}C°`}
            />
            <StatLine name="Time Passed" value={`${convertMinutes(mission.timePassed)} `} />
            <StatLine name="Time Estimated" value={`${convertMinutes(mission.timeEstimated)} `} />
          </div>
        ))}
      </div>
    </div>
